...
class HrAttendancePolicy(models.Model):
    ...
    overtime_policy_type = fields.Selection([('accumulate', 'Accumulate'), ('ontotal', 'On Total')], 'Policy Type', default="accumulate")
...
class HrOvertimeRule(models.Model):
    ...
    type = fields.Selection(selection=type, string="Type", default='workday')
    tolerance = fields.Float('Tolerance Time')
    active_after = fields.Float(string="Apply after",
                                help="After this time the overtime will be calculated")
    activate_from = fields.Float('From')
    activate_to = fields.Float('To')
    rate = fields.Float(string='Rate')

    @api.onchange('tolerance', 'active_after')
    def onchange_tolerance_active_after(self):
        for record in self:
            record.activate_from = record.active_after + record.tolerance

    @api.constrains('activate_from', 'activate_to')
    @api.onchange('activate_from', 'activate_to')
    def onchange_activate_from(self):
        for record in self:
            if record.activate_from > 24 or record.activate_to > 24:
                raise ValidationError(_("Sorry time can't exceed 24 hours"))
            if record.activate_from > record.activate_to and record.activate_from and record.activate_to:
                raise ValidationError(_("Sorry time from can't exceed time to"))

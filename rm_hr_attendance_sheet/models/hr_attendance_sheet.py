...
class AttendanceSheet(models.Model):
    ...
    total_act_diff_time = fields.Float(string="Total Actual Diff Time", required=False, compute="_compute_sheet_total", )
    ...
    def _compute_sheet_total(self):
        ...
        for sheet in self:
            ...
            sheet.total_planning = sum([l.pl_sign_out for l in planninglines]) - sum([l.pl_sign_in for l in planninglines])
            # Compute Absence
    ...
    def calc_overtime(self, float_overtime, policy_id, overtime_type):
        ...
        if policy_id.overtime_policy_type == 'accumulate':
            for wd_ot_obj in wd_ot_objs:
                overtime_level.append([wd_ot_obj.tolerance, wd_ot_obj.active_after, wd_ot_obj.activate_to, wd_ot_obj.rate])
            total_value_overtime = 0.0
            if float_overtime == 10.0:
                print(str("----"))
        ...
        #     if overtime_lev[0] <= float_overtime <= overtime_lev[1] and float_overtime >= overtime_lev[3]:
        #         total_value_overtime += (float_overtime - overtime_lev[0]) * overtime_lev[2]
        #         float_overtime = float_overtime - (float_overtime - overtime_lev[0])
        # if float_overtime and overtime_level[-1]:
        #     total_value_overtime += float_overtime * overtime_level[-1][2]
